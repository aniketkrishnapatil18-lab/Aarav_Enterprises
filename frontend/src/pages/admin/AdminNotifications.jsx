    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {loading ? (
          ...
          list.map(n => (
            <div key={n.id} className="glass-card" style={{ padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', opacity: n.is_read ? 0.6 : 1, borderLeft: n.is_read ? '1px solid var(--border-light)' : '3px solid var(--brand-violet)' }}>
              <div>
                <div style={{ fontWeight: 600, color: 'var(--text-main)', fontSize: '0.95rem', marginBottom: '0.2rem' }}>{n.title}</div>
                <div style={{ color: 'var(--text-subtle)', fontSize: '0.85rem', whiteSpace: 'pre-wrap' }}>{n.body}</div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-subtle)', marginTop: '0.3rem' }}>{timeAgo(n.created_at)}</div>
              </div>
              {!n.is_read && (
