    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: '#64748B', fontSize: '0.9rem' }}>{total} total inquiries</p>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
          {STATUSES.map(s => (
            <button key={s.value} onClick={() => setStatus(s.value)} style={{
              ...
              border: `1px solid ${status === s.value ? '#7C3AED' : 'rgba(255,255,255,0.1)'}`,
              background: status === s.value ? 'rgba(124,58,237,0.2)' : 'transparent',
              color: status === s.value ? '#A78BFA' : '#64748B', transition: 'all 0.2s',
            }}>{s.label}</button>
          ))}
        </div>
      </div>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {inquiries.map(inq => {
                ...
                return (
                  <tr key={inq.id}>
                    <td><span style={{ fontSize: '0.8rem', fontFamily: 'monospace', color: '#A78BFA' }}>{inq.inquiry_number}</span></td>
                    <td>
                      <div style={{ fontWeight: 600, color: '#F8FAFC', fontSize: '0.9rem' }}>{inq.customer_name || '—'}</div>
                      <div style={{ fontSize: '0.78rem', color: '#64748B' }}>{inq.whatsapp_number}</div>
                    </td>
                    <td><span style={{ fontSize: '0.875rem' }}>{inq.service_name || '—'}</span></td>
                    <td><span style={{ fontSize: '0.8rem', color: '#94A3B8' }}>{getLanguageLabel(inq.language)}</span></td>
                    <td><span className={`badge ${st.className}`}>{st.label}</span></td>
                    <td><span style={{ fontSize: '0.8rem', color: '#64748B' }}>{timeAgo(inq.created_at)}</span></td>
                    <td>
                      <Link to={`/admin/inquiries/${inq.id}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', color: '#A78BFA', textDecoration: 'none', fontSize: '0.85rem', fontWeight: 600 }}>
      ...
      {total > 20 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', marginTop: '1.5rem' }}>
          ...
          <span style={{ padding: '0.5rem 1rem', color: '#64748B', fontSize: '0.85rem' }}>Page {page} of {Math.ceil(total / 20)}</span>
